import { MicrophoneIcon, SearchIcon, XIcon } from '@heroicons/react/outline';
import Image from 'next/image'
import { useRouter } from 'next/router'
import React, { useRef, useState } from 'react'
import Avatar from '../components/Avatar';
import HeaderOptions from '../components/HeaderOptions';
import {signIn,signOut,useSession} from "next-auth/client"
import VoiceSearch from "../components/VoiceSearch"


function Header({e}) {
    
 const router = useRouter();
 const searchInputref = useRef('');
 const session = useSession();
 const [voicestate,setvoiceState] = useState(false)

 const search =(e)=>{
    const term= searchInputref.current.value;
    console.log("==>",term);
    if(!term) return;
  
    router.push(`/search?term=${term}`);



  }

    return (
        <header className="sticky  top-0 bg-gray-900 text-gray-100 z-999">
         <div  className="p-4 items-center flex w-full z-999">
         <Image loading="lazy" src='http://assets.stickpng.com/thumbs/580b57fcd9996e24bc43c51f.png'
            height={60}
            width={180} 
            onClick={()=>{router.push("/")}} 
            className="cursor-pointer"/>

            <form className="flex px-6 py-3 ml-18 mr-5 border border-gray-200 
            rounded-full shadow-lg max-w-3xl items-center flex-grow" >
                <input
                
                ref={searchInputref}
                className="flex-grow w-full focus:outline-none bg-gray-900 text-gray-100"
                 type="text"
               />

                <XIcon onClick={()=>{searchInputref.current.value=""}} className="h-7 sm:mr-3 text-gray-500 cursor-pointer transition
                duration-150 transform hover:scale-125"/>

                <MicrophoneIcon onClick={()=>setvoiceState(!voicestate)} className="h-7 mr-3 cursor-pointer  text-blue-500 bl-3 
                border-l-2 pl-4 border-gray-200 hidden sm:inline-flex"/>

               
                <SearchIcon onClick={()=>search(searchInputref.current.value)} className="h-7 mr-3 cursor-pointer text-blue-500  hidden sm:inline-flex "/>
                <button hidden type="submit" onClick={search} >Search</button>


            </form>

            <Avatar className='ml-auto z-1' url={  session[0]?.user?.image ?  session[0]?.user?.image:  "data:image/png;base64,"} ></Avatar>
            {voicestate &&  <VoiceSearch setvoiceState={setvoiceState} className="absolute z-1001    items-stretch h-screen w-screen  flex  bg-white " />}

         </div>
         <HeaderOptions e={e} />
        </header>
    )
}

export default Header
